...
// Usefull for hinting at a specific player
// format["Player: %1 - Selected role: %2", name player, _playerRole] remoteExec["hint"];
...
if(isServer) then {
	...
	while {true} do {
		...
		// SMG
		_crate addWeaponCargoGlobal ["SMG_02_F", 2];
		_crate addItemCargoGlobal [ "30Rnd_9x21_Mag_SMG_02", 30];
		_crate addItemCargoGlobal [ "optic_Aco_smg", 2];
		_crate addItemCargoGlobal [ "muzzle_snds_L", 1];


		// Base Rifle
		_crate addWeaponCargoGlobal ["SMA_Steyr_AUG_BLACK_F", 2];
		_crate addItemCargoGlobal [ "30Rnd_556x45_Stanag", 30];

		// Grenade Launcher Rifle 
		_crate addWeaponCargoGlobal [ "SMA_HK416GLCQB_B", 1];
		_crate addItemCargoGlobal [ "optic_Hamr", 1];
		_crate addItemCargoGlobal [ "1Rnd_HE_Grenade_shell", 7];
		_crate addItemCargoGlobal [ "1Rnd_SmokeBlue_Grenade_shell", 1];
		...
		_crate addItemCargoGlobal [ "200Rnd_65x39_cased_Box", 5];
